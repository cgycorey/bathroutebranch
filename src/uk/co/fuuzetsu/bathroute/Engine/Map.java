...
public class Map {
	...
	public Bitmap getBitmap(final Location l, Matrix m) {
		/*
		 * Canvas canvas = new Canvas(this.bmap); // new antialised Paint Paint
		 * paint = new Paint(Paint.ANTI_ALIAS_FLAG); // text color - #3D3D3D
		...
		if (this.containsLocation(l)) {
			...
			return Bitmap.createBitmap(this.bmap, cropX, cropY, ds.x, ds.y, m,
					true);

		} else {
			return Utils.decodeSampledBitmapFromResource(this.res, oobImg,
					ds.x, ds.y);
		}
	}
	...
	public Point locToMapPoint(final Location l) {
		...
		final Location tl = Utils.makeLocation(this.bl.getLongitude(),
				this.tr.getLatitude());

		final Integer newH = (int) (h.doubleValue() - h.doubleValue()
				* ((l.getLatitude() - this.bl.getLatitude()) / (this.tr
						.getLatitude() - this.bl.getLatitude())));

		final Integer newW = (int) (w.doubleValue() * ((l.getLongitude() - this.bl
				.getLongitude()) / (this.tr.getLongitude() - this.bl
				.getLongitude())));
